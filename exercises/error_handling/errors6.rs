...
enum ParsePosNonzeroError { // CreationError和ParseIntError都被ParsePosNonzeroError接收
...
fn parse_pos_nonzero(s: &str) -> Result<PositiveNonzeroInteger, ParsePosNonzeroError> {
    ...
    let x: i64 = match s.parse() { // 可能出现ParseIntError
        Ok(num) => num,
        Err(e) => { return Err(ParsePosNonzeroError::from_parseint(e)); },
    };
    match PositiveNonzeroInteger::new(x) { // 可能出现CreateionError
